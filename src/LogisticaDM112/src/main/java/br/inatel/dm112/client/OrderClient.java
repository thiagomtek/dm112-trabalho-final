...
public class OrderClient {
	...
	public void createOrder(Order order) {
		...
		WebClient.create(url).post().contentType(MediaType.APPLICATION_JSON)
				.header(HttpHeaders.ACCEPT, MediaType.APPLICATION_JSON_VALUE).body(Mono.just(order), Order.class)
	...
	public List<Order> getOrdersByCPF(String cpf) {
		...
		return WebClient.create(url).get().header(HttpHeaders.ACCEPT, MediaType.APPLICATION_JSON_VALUE).retrieve()
				.bodyToFlux(Order.class).collectList().log().block();
	}
	...
	public Order retrieveOrder(int orderNumber) {
		...
		return WebClient.create(url).get().retrieve().bodyToMono(Order.class).block();
	}
	...
	public void updateOrder(Order order) {
		...
		WebClient.create(url).put().contentType(MediaType.APPLICATION_JSON)
				.header(HttpHeaders.ACCEPT, MediaType.APPLICATION_JSON_VALUE).body(Mono.just(order), Order.class)
				.accept(MediaType.APPLICATION_JSON).retrieve();
	...
	private void makePUTCallWithEmptyBody(int orderNumber, String url) {
		try {
			HttpRequest request = HttpRequest.newBuilder().uri(new URI(url)).PUT(HttpRequest.BodyPublishers.noBody())
					.build();

			HttpResponse<String> response = HttpClient.newBuilder().build().send(request, BodyHandlers.ofString());
			if (response.statusCode() != 204) {
				throw new RuntimeException(
						"Error updating Order Delivery. statusCode: " + response.statusCode() + " expected: 204");
			}
		} catch (Exception e) {
			e.printStackTrace();
